...
function parseLiveGames(rows) {
  ...
  RANGES.forEach(range => {
    ...
    const sumScore = (players) => {
      ...
      return valid.reduce((s, p) => s + p.score, 0).toFixed(1);
    };
    ...
    const tbl = (players, name, score, nameColor) => `
      <div class="boxscore-card">
        ...
        <div class="boxscore-row header-row">
          <span>Player</span><span>Rank</span><span>Score</span>
        </div>
        ${players.filter(p => p.player).map(p => `
          <div class="boxscore-row">
            <span>${escapeHtml(p.player)}${p.captain
              ? ' <span style="color:#ff9f1c;font-size:10px;font-weight:700;background:rgba(255,159,28,0.15);border-radius:4px;padding:1px 5px;">CPT</span>'
              : ''}</span>
