...
function parseLiveGames(rows) {
  ...
  RANGES.forEach(range => {
    ...
    const buildPlayers = (rows, nameCol, rankCol) => rows.map(r => {
      const rawName = String(r[nameCol] || '').trim();
      if (!rawName) return null;

      const captain = /\sC$/.test(rawName);
      ...
      return {
        player: rawName,
        rank: isNaN(rawRank) ? '' : String(rawRank),
        score,
        captain
      };
    }).filter(Boolean);
    ...
    const sumScore = (players) => {
      ...
      return Math.round(valid.reduce((sum, p) => sum + p.score, 0));
    };
...
function renderLiveScoring(liveRows, scheduleRows) {
  ...
  if (!games.length) {
    els.liveRow.textContent = "No live games found.";
    return;
  }

  const scoreBadge = (score) =>
    score !== null ? `<span class="team-score">(${escapeHtml(score)})</span>` : '';
      ...
      <div class="boxscore-card">
        ...
        ${players.map(p => `
          <div class="boxscore-row">
            ...
            <span>${p.captain ? Math.round(p.score) + ' ✦' : escapeHtml(p.rank)}</span>
          </div>
        `).join('') || '<div class="boxscore-empty">No stats yet.</div>'}
      </div>`;
